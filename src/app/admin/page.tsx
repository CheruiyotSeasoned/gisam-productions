      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          ...
          <div className="sm:hidden divide-y">
            {(d.recent || []).map((r: any) => (
              <Link key={r.id} href={`/admin/applications?id=${r.id}`} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  <div className="font-medium text-secondary truncate">{r.full_name}</div>
                  <div className="text-xs text-slate-400">{r.reference}{r.county ? ` · ${r.county}` : ""}</div>
                </div>
                <StatusBadge status={r.status} />
              </Link>
            ))}
            {(!d.recent || d.recent.length === 0) && (
              <div className="px-5 py-8 text-center text-slate-400 text-sm">No applications yet.</div>
            )}
          </div>
        </Card>
